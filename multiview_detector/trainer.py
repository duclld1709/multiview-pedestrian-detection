...
class PerspectiveTrainer(BaseTrainer):
    ...
    def train(self, epoch, data_loader, optimizer, log_interval=100, cyclic_scheduler=None):
        ...
        for batch_idx, (data, map_gt, imgs_gt, _) in enumerate(data_loader):
            ...
            optimizer.step()
            losses += loss.item()
            map_prob = torch.sigmoid(map_res) if self._logit_output else map_res
            pred = (map_prob > self.cls_thres).int().to(map_gt.device)
    ...
    def test(self, data_loader, res_fpath=None, gt_fpath=None, visualize=False):
        ...
        for batch_idx, (data, map_gt, imgs_gt, frame) in enumerate(data_loader):
            ...
            map_prob = torch.sigmoid(map_res) if self._logit_output else map_res
            if res_fpath is not None:
                map_grid_res = map_prob.detach().cpu().squeeze()
        ...
        if visualize:
            ...
            subplt0.imshow(map_prob.cpu().detach().numpy().squeeze())
            ...
            imgs_prob0 = torch.sigmoid(imgs_res[0]) if self._logit_output else imgs_res[0]
            heatmap0_head = imgs_prob0[0, 0].detach().cpu().numpy().squeeze()
            heatmap0_foot = imgs_prob0[0, 1].detach().cpu().numpy().squeeze()
