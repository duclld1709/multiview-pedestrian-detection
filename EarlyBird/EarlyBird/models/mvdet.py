...
class MVDet(nn.Module):
    def __init__(
        ...
        self.mean = torch.as_tensor([0.485, 0.456, 0.406], device=device).reshape(
            1, 3, 1, 1
        )
        self.std = torch.as_tensor([0.229, 0.224, 0.225], device=device).reshape(
            1, 3, 1, 1
        )
        ...
        if self.num_cameras is None:
            ...
            self.world_feat = nn.Sequential(
                nn.Conv2d(
                    self.feat2d_dim * self.num_cameras,
                    latent_dim,
                    kernel_size=3,
                    padding=1,
                ),
    ...
    def forward(self, rgb_cams, pix_T_cams, cams_T_global, vox_util, ref_T_global):
        ...
        rgb_cams_ = (rgb_cams_ - self.mean.to(device)) / self.std.to(
            device
        )  # B*S,3,H,W
        ...
        featpix_T_ref_ = torch.matmul(
            featpix_T_cams_[:, :3, :3], cams_T_ref_[:, :3, [0, 1, 3]]
        )  # B*S,3,3
        ...
        # B*S,latent_dim,Y,X
        world_features_ = warp_perspective(
            feat_cams_, proj_mats, (self.Y, self.X), align_corners=False
        )
        ...
        if self.num_cameras is None:
            ...
            world_features = self.world_feat(
                world_features.view(B, S * self.feat2d_dim, self.Y, self.X)
            )
