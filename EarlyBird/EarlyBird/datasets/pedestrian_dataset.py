...
class PedestrianDataset(VisionDataset):
                ...
                for pedestrian in all_pedestrians:
                    ...
                    for cam in range(self.num_cam):
                        if itemgetter('xmin', 'ymin', 'xmax', 'ymax')(pedestrian['views'][cam]) != (-1, -1, -1, -1):
                            ...
                            num_imgs_bbox += 1
                # Keep 2D shapes even when a frame/camera has 0 boxes after drop.
                self.world_gt[frame] = (
                    np.asarray(world_pts, dtype=np.float32).reshape(-1, 2),
                    np.asarray(world_pids, dtype=np.int64).reshape(-1),
                )
                self.imgs_gt[frame] = {}
                for cam in range(self.num_cam):
                    # x1y1x2y2
                    self.imgs_gt[frame][cam] = (
                        np.asarray(img_bboxs[cam], dtype=np.float32).reshape(-1, 4),
                        np.asarray(img_pids[cam], dtype=np.int64).reshape(-1),
                    )
    ...
    def get_img_gt(self, img_pts, img_pids, sx, sy, crop):
        ...
        img_pts = np.asarray(img_pts, dtype=np.float32).reshape(-1, 4)
        img_pids = np.asarray(img_pids).reshape(-1)
    ...
    def __getitem__(self, index):
        ...
        worldgrid_pts = torch.as_tensor(np.asarray(worldgrid_pts, dtype=np.float32).reshape(-1, 2))
        world_pids = np.asarray(world_pids).reshape(-1)
        if worldgrid_pts.numel() == 0:
            worldgrid_pts = torch.zeros((0, 3), dtype=torch.float32).unsqueeze(0)
        else:
            worldgrid_pts = torch.cat((worldgrid_pts, torch.zeros_like(worldgrid_pts[:, 0:1])), dim=1).unsqueeze(0)

        if self.is_train and worldgrid_pts.shape[1] > 0:
        ...
        n_gt = min(worldgrid_pts.shape[1], self.max_objects)
        if n_gt > 0:
            grid_gt[:n_gt, :2] = worldgrid_pts[0, :n_gt, :2]
            grid_gt[:n_gt, 2] = torch.as_tensor(world_pids[:n_gt], dtype=torch.long)
        ...
        for cam_idx, (box, ids) in self.imgs_gt[frame].items():
            ...
            n = min(len(ids), self.max_objects)
            if n > 0:
                gt_boxes[cam_idx, :n] = torch.as_tensor(box[:n])
                gt_ids[cam_idx, :n] = torch.as_tensor(ids[:n])
