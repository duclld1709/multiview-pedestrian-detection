...
class DPerspTransDetector(nn.Module):
    ...
    def warp_perspective(self, img_feature_all):
        ...
        for j in range(6):
            ...
            for i in range(5):
                ...
                tt = warp_perspective(feat[None,None], self.proj_mats[i][j][None], self.reducedgrid_shape)[0,0]
    ...
    def forward(self, imgs, imgs_gt=None, map_gt=None, alpha=0, visualize=False):
        ...
        for i in range(5):
            tt = warp_perspective(t, self.proj_mats[i][0][None], self.reducedgrid_shape)
