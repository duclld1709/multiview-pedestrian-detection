...
def evaluateDetection_py(res_fpath, gt_fpath, dataset_name):
    ...
    # np.loadtxt collapses a file with exactly 1 row down to a 1-D array (shape (ncols,) instead
    # of (1, ncols)) -- happens routinely at a strict cls_thres under partial-annotation training,
    # where only a handful of detections survive across the whole test set. Restore the row axis
    # so the `[:, 0]`/`[idx, 1]` indexing below keeps working; skip when truly empty (shape (0,)).
